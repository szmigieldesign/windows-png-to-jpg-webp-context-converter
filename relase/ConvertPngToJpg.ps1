...
function Get-MagickExe {
    $bundledToolsRoot = Join-Path -Path $PSScriptRoot -ChildPath "tools\ImageMagick"
    if (Test-Path -LiteralPath $bundledToolsRoot) {
        $bundledMagick = Get-ChildItem -Path $bundledToolsRoot -Recurse -Filter "magick.exe" -File -ErrorAction SilentlyContinue |
            Select-Object -First 1
        if ($bundledMagick) {
            return [string]$bundledMagick.FullName
        }
    }
