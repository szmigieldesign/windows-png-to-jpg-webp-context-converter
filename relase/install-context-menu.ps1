...
$resolvedLauncherMode = $LauncherMode
if ($resolvedLauncherMode -eq "Auto") {
    $resolvedLauncherMode = if (Get-Command -Name "wscript.exe" -ErrorAction SilentlyContinue) { "Vbs" } else { "PowerShell" }
}
...
if ($resolvedLauncherMode -eq "Vbs" -and -not (Test-Path -LiteralPath $vbsLauncherPath)) {
    throw "VBS launcher not found: $vbsLauncherPath"
}

if ($resolvedLauncherMode -eq "PowerShell" -and -not (Test-Path -LiteralPath $psLauncherPath)) {
...
$hostCommand = if ($resolvedLauncherMode -eq "Vbs") {
